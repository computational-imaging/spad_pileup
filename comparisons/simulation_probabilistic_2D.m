function [] = simulation_probabilistic_2D()
%
% This code computes results for relevant comparison methods and the
% proposed probabilistic reconstruction method.
clear;
close all;
g = gpuDevice(1);
reset(g)
...
%Dataset
fdata = sprintf('../data/motorcycle_PicoquantBluePSF_compact.mat');
%fdata = sprintf('../data/motorcycle_GaussianPSF_compact.mat');

%Load
load(fdata, 'N_trials', 'd', 'I','sigma_true', 'data', 'buckets', ...
                   'psf_name', 'gm',  'a', 'z',  'QE', 'amb', 'DC', 'bucket_size', ...
                   'c', 'xt', 'xtpsf', 'T', 'times' );
sz = size(data.A);
...
if strcmp(psf_name, 'GaussianPSF')
    ...
    %Only use first Gaussian component
...
for j = print_rec
    fprintf('Log-Matched Filter: At pixel (%d) error in depth is %3.3f mm, %2.2f bins\n', j, abs(c*log_mean_naive(1,j)/2*1000 - d(1,j)*1000), abs(log_mean_naive(1,j)/bucket_size - z(1,j)))
end

%Display
if display_methods
    figure();
    imagesc(reshape(min(a,1),sz));
    axis image, colormap gray, colorbar;
    title('Ground truth amplitude')
    pause(0.5)
        
    figure();
    hssum = reshape(sum(buckets,1),sz);
    imagesc( hssum/max(hssum(:)) );
    axis image, colormap gray, colorbar;
    title('Histogram sum estimate of amplitude')
    pause(0.5)
    
    figure();
    imagesc(reshape(min(log_max_naive,1),sz));
    axis image, colormap gray, colorbar;
    title('Log-Matched amplitude')
    pause(0.5)
    
    figure();
    imagesc(abs( reshape(c*log_mean_naive/2 - d,sz) * 1000) .* (1-reshape(mask,sz)));
    axis image, colormap hot, caxis([0,15]), colorbar;
    title('Log-Matched depth error (mm)')
    pause(0.5)
    
    figure();
    imagesc( log(abs( reshape(c*log_mean_naive/2 - d,sz) * 1000) .* (1-reshape(mask,sz)) ));
    axis image, colormap hot, colorbar;
    title('Log-Matched log depth error (mm)')
    pause(0.5)
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Coates + Gauss Fit method
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for j = print_rec
    fprintf('Coates + Gaussfit: At pixel (%d) error in depth is %3.3f mm, %2.2f bins\n', j, abs(c*gaussmean_rc(1,j)/2*1000 - d(1,j)*1000), abs(gaussint_rc(1,j)/bucket_size - z(1,j)))
end
...
if display_methods
    ...
    axis image, colormap gray, colorbar;
    title('Coates + Gaussfit Coates amplitude')
    pause(0.5)
    
    figure();
    imagesc(abs( reshape(c*gaussmean_rc/2 - d,sz) * 1000) .* (1-reshape(mask,sz)));
    axis image, colormap hot, caxis([0,15]), colorbar;
    title('Coates + Gaussfit depth error (mm)')
    pause(0.5)
    
    figure();
    imagesc( log(abs( reshape(c*gaussmean_rc/2 - d,sz) * 1000) .* (1-reshape(mask,sz))) );
    axis image, colormap hot, colorbar;
    title('Coates + Gaussfit log depth error (mm)')
    pause(0.5)
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Probabilistic model per pixel (proposed method without spatial priors)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for j = print_rec
    fprintf('Probabilistic Newton: At pixel (%d) error in depth is %3.3f mm, %2.2f bins\n', j, abs(c*zv(1,j)*bucket_size/2*1000 - d(1,j)*1000), abs(zv(1,j) - z(1,j)))
end
...
if display_methods
    ...
    figure();
    imagesc(abs( reshape(zv*c*bucket_size/2 - d,sz) * 1000) .* (1-reshape(mask,sz)) );
    axis image, colormap hot, caxis([0,15]), colorbar;
    title('Probabilistic Newton depth error (mm)')
    pause(0.5)
    
    figure();
    imagesc( log(abs( reshape(zv*c*bucket_size/2 - d,sz) * 1000) .* (1-reshape(mask,sz)) ));
    axis image, colormap hot, colorbar;
    title('Probabilistic Newton log depth error (mm)')
    pause(0.5)
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Probabilistic spatio-temporal reconstruction (proposed method)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for j = print_rec
    fprintf('Probabilistic Newton TV: At pixel (%d) error in depth is %3.3f mm, %2.2f bins\n', j, abs(c*zprior(1,j)*bucket_size/2*1000 - d(1,j)*1000), abs(zprior(1,j) - z(1,j)))
end
...
if display_methods
    ...
    figure();
    imagesc(abs( reshape(zprior*c*bucket_size/2 - d,sz) * 1000) .* (1-reshape(mask,sz)) );
    axis image, colormap hot, caxis([0,15]), colorbar;
    title('Probabilistic Newton TV depth error [mm]')
    pause(0.5)
    
    figure();
    imagesc(log( abs( reshape(zprior*c*bucket_size/2 - d,sz) * 1000) .* (1-reshape(mask,sz)) ) );
    axis image, colormap hot,  colorbar;
    title('Probabilistic Newton TV log depth error [mm]')
    pause(0.5)
    
    figure();
    imagesc( reshape(z*c*bucket_size/2,sz) );
    axis image, colormap hsv, caxis([0,1]), colorbar;
    title('Ground truth depth map (RED pixels missing/ without depth information)');
    pause(0.5)
    
    figure();
    imagesc( reshape(zprior*c*bucket_size/2,sz) );
    axis image, colormap hsv, caxis([0,1]), colorbar;
    title('Probabilistic Newton TV final depth map');
    pause(0.5)
end
