...
%Dataset
fdata = './dataset/middlefield_depth_results/motorcycle_GaussianPSF_40.0.mat';
%fdata = './dataset/middlefield_depth_results/motorcycle_GaussianPSF_40.0_noalbedo.mat';
%fdata = './dataset/middlefield_depth_results/motorcycle_PicoquantBluePSF_40.0.mat';
%fdata = './dataset/middlefield_depth_results/motorcycle_PicoquantBluePSF_40.0_noalbedo.mat';
...
buckets(1:3,:) = 0;
buckets(end:-1:end-3,:) = 0;

%Potential subsampling
mask = mask(:)';
mask = mask(:,1:round(size(buckets,2)/50000):size(buckets,2));
d = d(:,1:round(size(buckets,2)/50000):size(buckets,2));
z = z(:,1:round(size(buckets,2)/50000):size(buckets,2));
buckets = buckets(:,1:round(size(buckets,2)/50000):size(buckets,2));
...
%{
figure();
imagesc(reshape(min(log_max,1),sz));
axis image, colormap gray, colorbar;
title('Log-matched Coates amplitude')

figure();
imagesc(reshape(c*log_mean/2,sz) );
axis image, colormap hsv, colorbar;
