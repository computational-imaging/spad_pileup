...
%Dataset
nd_filters = [0.001, 0.01, 0.05, 0.25, 0.5, 0.79, 1.0];
%nd_filters = [0.01, 0.05, 0.25, 0.5, 0.79, 1.0];
...
%##########Dataset###########
laser_types{end+1} = 'blue';
data_folders{end+1} = sprintf('../../data/experiment_9aug2017/pileup/near_%s_power_4.1', laser_types{end});
ambient(end+1) = 0.022;
%##########Dataset###########
laser_types{end+1} = 'blue';
data_folders{end+1} = sprintf('../../data/experiment_9aug2017/pileup/near_%s_power_4.6', laser_types{end});
ambient(end+1) = 0.02;
%##########Dataset###########
laser_types{end+1} = 'red';
data_folders{end+1} = sprintf('../../data/experiment_9aug2017/pileup/near_%s_power_1.8', laser_types{end});
ambient(end+1) = 0.03;
%##########Dataset###########
laser_types{end+1} = 'red';
data_folders{end+1} = sprintf('../../data/experiment_9aug2017/pileup/far_%s_power_1.75', laser_types{end});
ambient(end+1) = 0.03;
%##########Dataset###########
laser_types{end+1} = 'red';
data_folders{end+1} = sprintf('../../data/experiment_9aug2017/pileup/far_%s_power_1.76', laser_types{end});
ambient(end+1) = 0.03;
%##########Dataset###########
laser_types{end+1} = 'blue';
data_folders{end+1} = sprintf('../../data/experiment_9aug2017/pileup/far_%s_power_3.8', laser_types{end});
ambient(end+1) = 0.015;
...
for data_idx = 1:6
    ...
    %Close all
    close all;
    clearvars -except data_idx ambient data_folders laser_types dead_time nd_filters transmittance base_time acq_time
    
    %Dataset
    laser_type= laser_types{data_idx};
    data_folder= data_folders{data_idx};
    amb_setting = ambient(data_idx);

    %Print
    fprintf('\n#### Processing %s ####\n', data_folder)
    
    for ii = 1:length(nd_filters)
        fname{ii} = sprintf('%s/%s_diff_%.01f_sec_%.01f.mat', data_folder, laser_type, nd_filters(ii)*100, acq_time(ii)/1000);
    end
    %Plotting
    plotpad = [128,256];

    %Kai
    %acq_time = [40000,10000,3000,2000,1000,1000];
    ...
    for ii = low_indices
        if isempty(data_low)
            data_low = load(fname{ii}); 
            data_low.countsbuffer = double(data_low.countsbuffer(:));
        else
            cc = load(fname{ii}, 'countsbuffer'); 
            data_low.countsbuffer = cat(2, data_low.countsbuffer, double(cc.countsbuffer(:)));
        end
    end
...
function [zv, av] = fit_pileup(h, N, xt, gm, psf, QE, I, amb, DC)
    ...
    [x_val,~,~,~,~] = fmincon(objGrad_f,x0,[],[],[],[],[z0-20;0.01],[z0+20;100],[],options);
