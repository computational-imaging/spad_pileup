...
data_mat = data_mat[:150,150:170,160:180].astype('float64')
...
# G = np.cumsum(g)
NUM_TRIALS = 10000 # Number of shots
...
if False:
    ...
    buckets = np.load('hist.npy')[:150,150:170,160:180]
# print hist
...
def get_estimate(starts):
    start1, start2 = starts
    end1 = min(start1+STEP, dim1)
    end2 = min(start2+STEP, dim2)

    r_coates = np.zeros((T,end1-start1, end2-start2))
    r_recov = np.zeros((T,end1-start1, end2-start2))
    prob = 0
    r_vars = {}
    ...
    #         reg = cvx.sum_squares(r[1:] - r[:-1])
            ...
            r_coates[:,idx1-start1,idx2-start2] = r_c

    prob.solve(verbose=False, solver=cvx.ECOS, max_iters=200)

    for idx1 in range(start1,end1):
        for idx2 in range(start2,end2):
            r_star = r_vars[(idx1,idx2)].value.A.ravel()
            r_recov[:,idx1-start1,idx2-start2] = np.maximum(r_star, 0)

    return (r_coates, r_recov)

import multiprocessing
import itertools
pool = multiprocessing.Pool(processes=multiprocessing.cpu_count()-1)
starts = list( itertools.product(range(0,dim1,STEP), range(0,dim2,STEP)) )
solve_results = pool.map(get_estimate, starts)


r_coates_full = np.zeros((T,dim1,dim2))
r_recov_full = np.zeros((T,dim1,dim2))
for (r_c,r_star), (start1, start2) in zip(solve_results, starts):
    end1 = min(start1+STEP, dim1)
    end2 = min(start2+STEP, dim2)
    r_coates_full[:,start1:end1,start2:end2] = np.maximum(r_c, 0)
    r_recov_full[:,start1:end1,start2:end2] = np.maximum(r_star, 0)
    r_slc = data_mat[:,start1:end1,start2:end2]
...
for k, (idx1,idx2,label) in enumerate([(5,5,'Large box')]):
    for i, (name, image_arr) in enumerate(imgs.items()):
